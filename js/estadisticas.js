...
/**
 * Función de diagnóstico para verificar el estado de las gráficas y contenido
 */
function diagnosticarEstadisticas() {
    console.log('🔍 === DIAGNÓSTICO DE ESTADÍSTICAS ===');
    
    // Verificar contenedor principal
    const contenedor = document.querySelector('.estadisticas-container');
    console.log('📦 Contenedor principal:', contenedor ? 'ENCONTRADO' : 'NO ENCONTRADO');
    if (contenedor) {
        console.log('📏 Dimensiones contenedor:', {
            width: contenedor.offsetWidth,
            height: contenedor.offsetHeight,
            visible: contenedor.offsetHeight > 0 && contenedor.offsetWidth > 0
        });
    }
    
    // Verificar tarjetas
    const tarjetas = document.querySelectorAll('.estadistica-card');
    console.log(`🃏 Tarjetas encontradas: ${tarjetas.length}`);
    tarjetas.forEach((tarjeta, i) => {
        console.log(`  Tarjeta ${i + 1}:`, {
            texto: tarjeta.textContent.trim().substring(0, 50),
            visible: tarjeta.offsetHeight > 0
        });
    });
    ...
    canvases.forEach((canvas, i) => {
        ...
        for (let j = 3; j < data.length; j += 4) {
            if (data[j] > 0) pixelesNoTransparentes++;
        }
        
        console.log(`  Canvas ${i + 1}:`, {
            id: canvas.id,
            dimensiones: `${canvas.width}x${canvas.height}`,
            pixelesContenido: pixelesNoTransparentes,
            tieneContenido: pixelesNoTransparentes > 100,
    ...
    console.log('📊 Tabla de resumen:', tabla ? 'ENCONTRADA' : 'NO ENCONTRADA');
    if (tabla) {
    ...
    console.log('📄 html2pdf disponible:', typeof html2pdf !== 'undefined' ? 'SÍ' : 'NO');
